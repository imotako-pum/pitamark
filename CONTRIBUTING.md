...
snap-share は個人 OSS で、**本番 deploy はオーナー (`@imotako-pum`) のみ** が実行します。
PR 作者は `wrangler deploy` を呼ぶ必要はありません（`CLOUDFLARE_API_TOKEN` を持たないので呼べません）。

運用方針（[PRD Decisions Log](./.claude/PRPs/prds/snap-share.prd.md#decisions-log) で確定）:

- **API (Workers)**: オーナーが手動で `cd apps/api && pnpm wrangler deploy`
- **Web (Pages)**: `main` ブランチ push で Cloudflare Pages が自動 build
- **観測**: Cloudflare Web Analytics + `wrangler tail` のみ（外部 APM なし）

具体手順とランブックは [README#production-deploy](./README.md#production-deploy)、
KPI / SLO / 撤退ラインは [docs/observability.md](./docs/observability.md) を参照。
