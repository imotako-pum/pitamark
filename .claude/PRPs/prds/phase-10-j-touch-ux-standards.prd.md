...
|---|-------|-------------|--------|----------|---------|----------|
| 10.J-1 | paired binding 規約適用 + タイミング定数 SSOT | ADR-0007 起票、`apps/web/src/lib/touch-thresholds.ts` 新設、全 4 Shape の `onClick + onTap` / `onDblClick + onDblTap` 配線 + Circle handle の `onTouchStart`、unit test で paired binding assertion 追加、`Event.isTrusted` の grep 確認 | complete (typecheck/lint/test 354 件 (paired binding 8 件追加)/220 files biome ci すべて緑、isTrusted 参照ゼロ件確認済) | - | - | [plan](../plans/phase-10-j-1-paired-binding-and-touch-thresholds.plan.md) |
| 10.J-2 | 長押しコンテキストメニュー実装 | `useLongPress` hook + `ContextMenu.tsx` + 4 項目メニュー (削除 / 複製 / 前面 / 背面)、誤発火対策 (500ms + 6px slop + opacity fade は 10.J-3 で別途)、`navigator.vibrate(15)` Android only。z-order は **案 B** (`createdAt` 流用) で schema 変更回避。AnnotationLayer ではなく EditorShell に menu state を hold して dispatch | complete (typecheck/lint/test 390 件 (10.J-2 は +28 件: useLongPress 8 / ContextMenu 8 / operations 9 / reducer 5 / yjs-mutations 6 - reducer 5 は reorder 4 + isCommitting reorder 1)/biome ci 226 files すべて緑) | with 10.J-3 | 10.J-1 | [plan](../plans/phase-10-j-2-long-press-context-menu.plan.md) |
| 10.J-3 | Transformer coarse anchor 再調整 + サイズ定数整合 | `colors.ts` の `ANCHOR_SIZE_TOUCH 24 → 20`、`MIN_TAP_TARGET_PX` / `HIT_TEST_MARGIN_PX` の定数化、touch device での visual 検証 | complete (typecheck/lint/test 401 件 (10.J-3 は +11 件: touch-thresholds 7 / colors 4)/biome ci 226 files すべて緑、mobile-chrome E2E は単独実行で全緑、並列負荷の flaky setupEditor は既知) | with 10.J-2 | 10.J-1 | [plan](../plans/phase-10-j-3-transformer-coarse-anchor-and-size-constants.plan.md) |
| 10.J-4 | E2E migration + 実機 QA + umbrella report | `touchSequence` helper 拡張、既存 19 spec を `dispatchEvent` 経路に migration、新規 spec (paired binding + long-press menu) 追加、`docs/qa/phase-10-j-touch-manual-qa.md` 拡張、iPhone Safari + Android Chrome 実機 QA 100% 通過、Phase 10.J 全体の umbrella report | complete (CI auto: typecheck/lint/test 401 件/biome ci 228 files/E2E mobile-chrome 31 件 + chromium 78 件すべて緑、実機 QA は docs/qa 著者消費待ち) | - | 10.J-2, 10.J-3 | [plan](../plans/completed/phase-10-j-4-e2e-migration-and-manual-qa.plan.md) |
...
*umbrella report: [`phase-10-j-umbrella-report.md`](../reports/phase-10-j-umbrella-report.md)*
