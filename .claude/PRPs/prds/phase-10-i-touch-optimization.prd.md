...
|---|-------|-------------|--------|----------|---------|----------|
| 10.I-1 | Pointer Events 一本化 + 描画系復旧 | `CanvasStage` を `onPointerDown/Move/Up` 化、`touch-action: none`、4 形状の touch drag 復旧、unit test を pointer 系に移行、ADR-0006 起票 | pending | - | - | - |
