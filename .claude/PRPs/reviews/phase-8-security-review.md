...
|---|---|---|
| **H1** WebSocket JWT が URL query param 経由で露出 | 30s 一発限り KV ticket 化 (32-hex)。`POST /rooms/:id/ws-ticket` で JWT bearer を ticket に交換、`?ticket=<hex>` で WS upgrade。consume 時に KV.delete で replay 不可 | `apps/api/src/services/ws-ticket-service.ts` (new) / `apps/api/src/routes/rooms.ts` / `apps/api/src/yjs.ts` / `apps/api/src/lib/bindings.ts` (`WS_TICKETS`) / `apps/api/wrangler.toml` / `apps/web/src/lib/api-client.ts` (`requestWsTicket`) / `apps/web/src/hooks/useYjsAnnotationsStore.ts` |
