...
**Date**: 2026-05-05
**Status**: accepted (Option B 軽量自作 dict + 日英 2 言語、Phase 10.E で実装)
**Deciders**: imotako (PM/Dev)
**Related**: `.claude/PRPs/prds/snap-share.prd.md` / `.claude/PRPs/prds/phase-10-direction.prd.md` / Phase 10.E (i18n 軽量実装) / [ADR-0003](./ADR-0003-web-vs-desktop-direction.md) (形態方針 = on hold)
...
**Option B (軽量自作 dict + 日英 2 言語) を採用**（2026-05-05 Q5 で確定、Status: accepted）。

ADR-0003 が on hold (Mac spike Phase 11+ 後回し) になり Web 単独確定したため、本 ADR の待ち条件が解消し独立実行可能となった。Phase 10.E で実装する。

### Option B（採用）: 軽量自作 dict + 日英 2 言語のみ
...
## Validation / Acceptance（Status: accepted の確認事項、すべて完了済）

- [x] ADR-0003 の Decision が出ている → **on hold (Web 単独確定)**、本 ADR 独立実行可
- [x] Mac 主軸採用時の Tauri 標準 i18n との併用方針 → **Phase 11+ で Mac 再検討時に別途、本 ADR は web 側 i18n に集中**
- [ ] 推定 100-200 キーの英訳がオーナー単独で対応可能か確認 (Phase 10.E 実装時に DeepL/GPT 併用前提)
- [ ] 言語切替 UI のデザインが既存 UI を破壊しないか確認 (Phase 10.E 実装時)
