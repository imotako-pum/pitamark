...
**Date**: 2026-05-05
**Status**: on hold (Mac spike を Phase 11+ 候補へ後回し、2026-05-05 オーナー判断)
**Deciders**: imotako (PM/Dev)
**Related**: `.claude/PRPs/prds/snap-share.prd.md` / `.claude/PRPs/prds/phase-10-direction.prd.md` / ~~Phase 10.C (Mac spike)~~ → Phase 11+ 候補
...
**Phase 10 では Web 単独継続 (Option 1) を一時的に採用**。Mac spike および 3 形態判断は **Phase 11+ 候補へ後回し** (Status: on hold)。

### 2026-05-05 オーナー判断 (Q9)

> 「Mac はやっぱ後回しだな。一旦全部 Web で。これまでの回答も Mac Spike はひとまずやらない方向で」
> 「製品化は確実に Web が先」

### 後回し理由

1. **Phase 10 の集中度確保**: Mac spike (1-2 週間) を入れると公開リリース + i18n + アプリ名/ドメイン取得との並走で集中度が下がる
2. **Web 単独で先に製品化が済む**: Phase 8.x まで完了済の Web 版を **公開リリース → Analytics 観察 → 実数字** で評価する方が判断材料の質が高い
3. **Mac spike の前提が変わる可能性**: 公開後 1 ヶ月の数字次第で「そもそも Mac 化する価値があるか」「どの形態を spike すべきか」自体が再定義される
4. **既存 spike 設計は資産として保存**: 本 ADR の Spike 設計セクションをそのまま Phase 11+ 再開時に流用可能

### Phase 11+ 再検討トリガー

以下のいずれかが Phase 10.G 観察期間 (1 ヶ月) で観測された場合、本 ADR を再オープンして Spike 実施を判断:

- [ ] 公開後の Analytics で「Mac ユーザー比率 > 50%」(Cloudflare Web Analytics の OS 比率)
- [ ] dogfood の「詰まった瞬間メモ」で「ブラウザだとキャプチャ→注釈ワークフローが面倒」が複数回出現
- [ ] 競合 (Shottr / CleanShot X) のユーザーから「snap-share の Mac 版欲しい」直接フィードバック
- [ ] C スタンス維持判断 (Phase 11 起票時) で「フリーミアム差別化として Mac 版が必要」と結論

### Phase 11 で再開する場合の参照

- 本 ADR の **Spike 設計**, **Alternatives Considered**, **Consequences** セクションがそのまま流用可能
- Tauri 2.0 のバージョン進化を再確認 (本 ADR 起票時点 = 2.0)
- ADR-0004 の i18n 戦略 (Phase 10.E 完了済) との併用方針を別途検討

---

### (元の 3 形態案 — Phase 11+ 再開時の参照用)

将来 Spike を実施する場合、以下 3 形態のいずれかを Status: accepted で確定する:
...
## Validation / Acceptance（Status: on hold → 再オープン → accepted へ昇格させる前に確認する事項）

### Phase 11+ で再オープンする条件 (上記 Phase 11+ 再検討トリガーのいずれか)

- [ ] Mac ユーザー比率 > 50% を観察
- [ ] dogfood メモで「ブラウザだとワークフロー面倒」が複数回出現
- [ ] 競合ユーザーから Mac 版要求フィードバック
- [ ] C スタンス維持判断時に「Mac 版がフリーミアム差別化に必要」と結論

### Spike 実施後 (再開時) に Status: accepted へ昇格させる前に確認する事項

- [ ] Phase 11+ で計画される Mac spike の完了
- [ ] Go/No-Go Gate（1 週目）の 4 項目のうち 3 項目以上 OK
- [ ] Success Signal（2 週目）の golden path 動作
- [ ] オーナーの体感ベース判断（「Mac 版を毎日使いたいか」）が記録されている
- [ ] 選んだ Option（1 / 2 / 3）の Phase 11+ 実装方針への影響が記録されている
